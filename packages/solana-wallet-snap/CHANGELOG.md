...
- Accept non-URL request origins instead of throwing while rendering confirmations, so requests relayed over WalletConnect (whose origin is a per-session channel id) can be confirmed ([#278](https://github.com/MetaMask/internal-snaps/pull/278))
- Show the dapp URL reported in `originMetadata` in the "Request from" row of the transaction, sign-message, and sign-in confirmations, marked "Not verified", and hide the row entirely when there is nothing verifiable or self-reported to show (previously an unverifiable origin was displayed as if verified, or threw) ([#278](https://github.com/MetaMask/internal-snaps/pull/278))
- Stop sending unverifiable origins to the security alerts API: a WalletConnect channel id or a self-reported dapp URL can flip a Blockaid verdict, so such requests are now reported as wallet-initiated ([#278](https://github.com/MetaMask/internal-snaps/pull/278))
- Only run the sign-in (SIWS) domain check against a verifiable origin, instead of comparing the requested domain to a self-reported one ([#278](https://github.com/MetaMask/internal-snaps/pull/278))
