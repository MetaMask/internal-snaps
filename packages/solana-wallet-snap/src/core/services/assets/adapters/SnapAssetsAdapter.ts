...
export class SnapAssetsAdapter {
  ...
  #splitAssetsByType(assetTypes: CaipAssetType[]): {
    ...
    const nativeAssetTypes = assetTypes.filter((assetType): boolean =>
      assetType.endsWith(SolanaCaip19Tokens.SOL),
    ) as NativeCaipAssetType[];
    const tokenAssetTypes = assetTypes.filter((assetType): boolean =>
      assetType.includes('/token:'),
    ) as TokenCaipAssetType[];
    const nftAssetTypes = assetTypes.filter((assetType): boolean =>
