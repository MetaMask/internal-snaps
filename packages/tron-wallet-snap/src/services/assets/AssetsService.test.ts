...
async function withAssetsService<ReturnValue>(
  ...
  const mockCoreMessenger: jest.Mocked<CoreMessengerCaller> = {
    call: jest
      .fn()
      .mockImplementation(createMessengerCallMock(mockGetAsset, mockGetAssets)),
  };
...
describe('AssetsService', () => {
  ...
  describe('AssetsController routing', () => {
    ...
    it('fetchAssetsAndBalancesForAccount returns snap-owned assets only', async () => {
      await withAssetsService(
        async ({
          assetsService,
          mockTrongridApiClient,
    ...
    it('fetchAssetsAndBalancesForAccount returns protocol assets only for inactive accounts', async () => {
      await withAssetsService(
        async ({
          assetsService,
          mockTrongridApiClient,
    ...
    it('routes fungible reads through AssetsController', async () => {
      await withAssetsService(async ({ assetsService, mockCoreMessenger }) => {
        mockCoreMessenger.call.mockImplementation(
          createMessengerCallMock(jest.fn().mockResolvedValue(
    ...
    it('getAccountAssetsByIDs uses a single AssetsController:getAssets call for fungibles', async () => {
      await withAssetsService(async ({ assetsService, mockCoreMessenger }) => {
        ...
        mockCoreMessenger.call.mockImplementation(
          createMessengerCallMock(jest.fn(),
            jest.fn().mockImplementation(async (_accounts, _options) => {
              return {
                [accountId]: {
    ...
    it('getAccountAssetsByIDs merges snap-owned and fungible reads in request order', async () => {
      await withAssetsService(
        async ({ assetsService, mockAssetsRepository, mockCoreMessenger }) => {
          ...
          mockCoreMessenger.call.mockImplementation(
            createMessengerCallMock(jest.fn(),
              jest.fn().mockImplementation(async (_accounts) => {
                return {
                  [accountId]: {
