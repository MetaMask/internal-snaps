...
type WithSnapAssetsAdapterCallback<ReturnValue> = (payload: {
  adapter: SnapAssetsAdapter;
...
/**
 * Wraps tests for SnapAssetsAdapter by creating a fresh adapter with all mock
 * dependencies. The callback receives the adapter and all mocks for
 * test configuration.
 *
 * @param testFunction - The test body receiving the adapter and mocks.
 * @returns The return value of the callback.
 */
async function withSnapAssetsAdapter<ReturnValue>(
  testFunction: WithSnapAssetsAdapterCallback<ReturnValue>,
): Promise<ReturnValue> {
  ...
  const adapter = new SnapAssetsAdapter({
    logger: mockLogger,
...
describe('SnapAssetsAdapter', () => {
  describe('fetchAssetsAndBalancesForAccount', () => {
    describe('inactive account fallback', () => {
      ...
      it('returns protocol resources when inactive account has empty resources', async () => {
        await withSnapAssetsAdapter(
          async ({ adapter, mockTrongridApiClient, mockTronHttpClient }) => {
            mockTrongridApiClient.getAccountInfoByAddress.mockRejectedValue(
              new TrongridAccountNotFoundError(),
            );
            mockTronHttpClient.getAccountResources.mockResolvedValue(
  ...
  describe('saveMany', () => {
    it('does not remove energy and bandwidth assets even when they have zero amounts', async () => {
      await withSnapAssetsAdapter(
        async ({ adapter, mockState, mockAssetsRepository }) => {
    ...
    it('correctly updates non-essential assets with zero amounts', async () => {
      await withSnapAssetsAdapter(async ({ adapter, mockState }) => {
    ...
    it('updates stale non-essential assets balance to 0 if missed from the latest snapshot', async () => {
      await withSnapAssetsAdapter(
        async ({ adapter, mockState, mockAssetsRepository }) => {
          ...
          await adapter.saveMany(updatedAssets);
  ...
  describe('getAssetsMetadata', () => {
    it('resolves metadata for native, protocol, and token asset types', async () => {
      await withSnapAssetsAdapter(async ({ adapter, mockTokenApiClient }) => {
        const trc20 =
        ...
        const metadata = await adapter.getAssetsMetadata(assetTypes);
  ...
  describe('isFiat', () => {
    it('returns true only for fiat CAIP asset IDs', () => {
      expect(SnapAssetsAdapter.isFiat('eip155:1/erc20:0x0')).toBe(false);
      expect(SnapAssetsAdapter.isFiat('swift:0/iso4217:usd')).toBe(true);
    });
  });

  describe('hasChanged', () => {
    it('returns true when the asset is missing or the amount differs', () => {
      ...
      expect(SnapAssetsAdapter.hasChanged(asset, [])).toBe(true);
      expect(SnapAssetsAdapter.hasChanged(asset, [asset])).toBe(false);
    });
  });
});
