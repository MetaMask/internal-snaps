...
- Show the dapp URL reported in `originMetadata` in the "Request from" row of the sign-transaction and sign-message confirmations, marked "Not verified", and hide the row when there is nothing verifiable or self-reported to show (previously a WalletConnect channel id was displayed verbatim as the requesting site) ([#280](https://github.com/MetaMask/internal-snaps/pull/280))
- Stop sending unverifiable origins to the security alerts API: a WalletConnect channel id or a self-reported dapp URL can flip a Blockaid verdict, so such requests are now reported as wallet-initiated ([#280](https://github.com/MetaMask/internal-snaps/pull/280))
