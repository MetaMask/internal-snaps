...
- Return a typed zero balance entry from `getAccountBalances` when a requested asset id is missing from the on-chain snapshot (or the account is inactive), instead of omitting the key ([#367](https://github.com/MetaMask/internal-snaps/pull/367))
