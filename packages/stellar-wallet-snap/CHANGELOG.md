...
- Add send confirmation memo edit UI for SEP-29 RequiresMemo recovery: store memo on confirmation context (`context.memo`), resolve dialog as `{ confirmed, memo? }`, and rebuild with `skipMemoRequirementCheck` / post-confirm memo attach (no client wire `memo` / `memoType` params) ([#323](https://github.com/MetaMask/internal-snaps/pull/323))
