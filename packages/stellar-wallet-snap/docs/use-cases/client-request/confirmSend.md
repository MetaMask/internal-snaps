...
Memo is **not** a client wire param. The confirmation UI owns memo on interface context (`context.memo`); the dialog resolves with `{ confirmed, memo? }`, and that value is passed into post-confirm / refresh rebuilds via `createValidatedSendTransaction({ memo })`.
...
**RequiresMemo (SEP-29):** when the destination requires a memo and none was provided, the snap opens a send confirmation on a draft envelope (`skipExceptions: [RequiresMemoException]`) with a banner asking the user to add a memo. The confirm UI includes an Add/Update memo row and edit screen that stores the memo on confirmation **context** (`context.memo`), not on `request.params`. After the user confirms with a memo, the handler re-validates (including the memo requirement) via the existing post-confirm refresh path before signing. Confirming without a memo still returns `{ valid: false, errors: [{ code: Invalid }] }`. While the dialog is open, live refresh **pauses** on `RequiresMemo` (omit scan this cycle, pause auto-cron); MemoEdit Save cancel-and-replaces to restart validation and security scanning.
...
4. **Pre-submit validation errors** — Balance, trustline, and create-account failures are shown in the send confirmation (no fee or price estimates). After the dialog is dismissed, the handler throws `UserRejectedRequestError`. `RequiresMemo` opens a send confirmation instead (draft + banner) so the user can add a memo.
...
sequenceDiagram
  ...
  loop While dialog open (until Client confirms / rejects)
    ...
    Refresh->>Tx: rebuild & re-validate send tx (memo from context.memo)
    Refresh->>Scan: security scan (refreshed envelope)
    Refresh->>UI: update confirmation context
  end

  Client-->>UI: confirm
  UI-->>Handler: { confirmed, memo? }
  Handler->>Resolver: resolve again (live on-chain)
  Handler->>Tx: createValidatedSendTransaction({ memo })
