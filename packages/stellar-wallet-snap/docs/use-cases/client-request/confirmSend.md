...
**RequiresMemo (SEP-29):** when the destination requires a memo and none was provided, the snap opens a recoverable send confirmation on a draft envelope (`skipExceptions: [RequiresMemoException]`) with a banner asking the user to add a memo. The confirm UI includes an Add/Update memo row and edit screen that stores the memo on confirmation **context** (`context.memo`), not on `request.params`. After the user confirms with a memo, the handler re-validates (including the memo requirement) via the existing post-confirm refresh path before signing. Confirming without a memo still returns `{ valid: false, errors: [{ code: Invalid }] }`. While the dialog is open, live refresh treats `RequiresMemo` as recoverable (`recoverable: true`) so saving a memo can restart validation and security scanning without nulling `securityScanRequest`.
