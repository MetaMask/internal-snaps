...
import type {BitcoinAccountRepository, BitcoinAccount, SnapClient, Inscription, AccountState, AccountMetadata, SnapState} from '../entities';
...
export class BdkAccountRepository implements BitcoinAccountRepository {
  ...
  async insertMany(accounts: BitcoinAccount[]): Promise<BitcoinAccount[]> {
    ...
    if (accounts.length === 1) {
      return [await this.insert(accounts[0])];
    }
